...
import { SidebarCollapseIcon } from "@/components/icons/sidebar-icons"
    ...
    >
      ...
      >
        <div
          ...
          className="flex size-full flex-col bg-sidebar/95 backdrop-blur-2xl group-data-[variant=floating]:rounded-xl group-data-[variant=floating]:shadow-lg group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
        >
          {children}
        </div>
      </div>
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium tracking-wide text-sidebar-foreground/65 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
