...
const authEmailRateLimits = new Map<
  string,
  { count: number; resetAt: number }
>()
const AUTH_EMAIL_RATE_LIMIT_MAX = 3
const AUTH_EMAIL_RATE_LIMIT_WINDOW_MS = 15 * 60 * 1000
...
async function checkAuthEmailRateLimit(action: string, email: string) {
  ...
  const clientIp =
    ...
    "unknown"
  const key = `${action}:${email.toLowerCase()}:${clientIp}`
  const now = Date.now()
  const entry = authEmailRateLimits.get(key)

  if (!entry || now > entry.resetAt) {
    authEmailRateLimits.set(key, {
      count: 1,
      resetAt: now + AUTH_EMAIL_RATE_LIMIT_WINDOW_MS,
    })
    return true
  }

  if (entry.count >= AUTH_EMAIL_RATE_LIMIT_MAX) return false
  entry.count++
  return true
}
